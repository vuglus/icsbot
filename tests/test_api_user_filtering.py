...
def test_api_with_user_filtering():
    """Test the API with user filtering"""
    ...
    try:
        ...
        response = requests.get(
            'http://localhost:5000/events/pending',
        ...
        response = requests.get(
            'http://localhost:5000/events/pending?user_id=user1',
            headers={'X-API-Key': 'test-api-key'}
        )
        ...
        response = requests.get(
            'http://localhost:5000/events/pending?user_id=user2',
            headers={'X-API-Key': 'test-api-key'}
        )
        ...
        response = requests.get(
            'http://localhost:5000/events/pending?user_id=nonexistent',
            headers={'X-API-Key': 'test-api-key'}
        )
